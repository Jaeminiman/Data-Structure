...
public class MovieDatabaseConsole {
	...
	public static void main(String args[]) throws IOException {
		...
		while (true) {
			try {
				...
				//args 할당된 command 객체 생성
				//if (command instanceof InsertCmd){
				//	System.out.println("Insert");
				//}
				//else if (command instanceof DeleteCmd){
				//	System.out.println("Insert");
				//}
				//else if (command instanceof PrintCmd){
				//	System.out.println("Print");
				//}
				//else if (command instanceof SearchCmd){
				...
				// 명확하게 드러낼 수 있다.
				// 해당 Console에서는 입력 및 해석을 진행하고, 처리는 Console Command Interface에 맡긴다.
	...
	private static ConsoleCommand parse(String input) throws Exception {
		// 우선 어떤 종류로 ConsoleCommand(인터페이스)를 class로 구현할 것인지 결정한다.
		...
		/*
		 * ConsoleCommand(인터페이스)의 구현을 위한 Class 종류가 결정되었으니 입력을 각 Class의 방식에 맞춰
		 * 해석(parse)한다.
		 */
